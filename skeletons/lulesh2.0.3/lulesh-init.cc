   ...
   m_refdens(Real_t(1.0))
{
   ...
   const Real_t_sim ebase = Real_t(3.948746e+7);
   Real_t_sim scale = (nx*Index_t_sim_conv(m_tp))/Real_t(45.0);
...
Domain::BuildMesh(Int_t nx, Int_t edgeNodes, Int_t edgeElems)
{
  ...
  Index_t nidx = 0 ;
  Real_t_sim tz = Real_t(1.125)*Real_t_sim(Index_t_sim_conv(m_planeLoc)*nx)/Real_t_sim(meshEdgeElems) ;
  for (Index_t plane=0; plane<edgeNodes; ++plane) {
    Real_t_sim ty = Real_t(1.125)*Real_t_sim(Index_t_sim_conv(m_rowLoc)*nx)/Real_t_sim(meshEdgeElems) ;
    for (Index_t row=0; row<edgeNodes; ++row) {
      Real_t_sim tx = Real_t(1.125)*Real_t_sim(Index_t_sim_conv(m_colLoc)*nx)/Real_t_sim(meshEdgeElems) ;
	...
	tx = Real_t(1.125)*Real_t_sim(Index_t_sim_conv(m_colLoc)*nx+col+1)/Real_t_sim(meshEdgeElems) ;
      }
      // ty += ds ;  // may accumulate roundoff... 
      ty = Real_t(1.125)*Real_t_sim(Index_t_sim_conv(m_rowLoc)*nx+row+1)/Real_t_sim(meshEdgeElems) ;
    }
    // tz += ds ;  // may accumulate roundoff... 
    tz = Real_t(1.125)*Real_t_sim(Index_t_sim_conv(m_planeLoc)*nx+plane+1)/Real_t_sim(meshEdgeElems) ;
  }
